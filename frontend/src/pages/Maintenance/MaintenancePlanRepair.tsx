...
import {
  ...
  useMaintenanceYearSummaryQuery,
} from "./mutation";
import { AssetGroup, CongTy } from "../../utils/const";
import { MaintenancePlanData } from "./types";
import { showServerity, showShareStatus, showStatus } from "./config";
...
export default function MaintenancePlanRepair() {
  ...
  // ── kehoach: 2 tầng — danh sách năm (tóm tắt) + kế hoạch theo năm đang mở ──
  const [yearPage, setYearPage] = useState(0);
  const [yearPageSize] = useState(5);
  const [expandedYear, setExpandedYear] = useState<number | null>(null);
  const [yearPlansPage, setYearPlansPage] = useState(0);
  const [yearPlansPageSize] = useState(10);

  const { data: yearSummary, isLoading: isLoadingYears } =
    useMaintenanceYearSummaryQuery(
      CongTy.CT001,
      yearPage,
      yearPageSize,
  ...
  const yearItems: {
    ...
    soLuongThietBi: number;
  }[] = (yearSummary?.data?.items ?? yearSummary?.items ?? []) as {
    nam: number;
    soLuongKeHoach: number;
    soLuongThietBi: number;
  }[];
  const yearTotal: number = (yearSummary?.data?.totalItems ??
    yearSummary?.totalItems ??
    0) as number;
  const yearTotalPages = Math.max(1, Math.ceil(yearTotal / yearPageSize));

  const { data: yearPlansData, isLoading: isLoadingYearPlans } =
    useMaintenancePlanningPageQuery(
      yearPlansPage,
      yearPlansPageSize,
      searchDebounce,
      statusFilter !== "" ? Number(statusFilter) : undefined,
      expandedYear ?? undefined,
      selectedDepartment,
      ...
      nhomTaiSanFilter,
      undefined,
      expandedYear !== null && activeTab === 0,
    );

  const currentYearPlans: MaintenancePlanData[] = (yearPlansData?.data?.items ??
    yearPlansData?.items ??
    []) as MaintenancePlanData[];
  const currentYearPlansTotal: number = (yearPlansData?.data?.totalItems ??
    yearPlansData?.totalItems ??
    0) as number;
  const currentYearPlansTotalPages = Math.max(
    1,
    Math.ceil(currentYearPlansTotal / yearPlansPageSize),
  );

  // Reset phân trang mỗi khi filter thay đổi
  useEffect(() => {
    setYearPage(0);
  ...
  const {
    data: incidentPageData = { items: [], totalItems: 0 },
  ...
  const handleEditIncidentClick = (incident: any, e: React.MouseEvent) => {
    setSelectedIncident(incident);
    setShowIncidentDialog(true);
    e.stopPropagation();
  };

  const handleDeleteIncidentClick = (incident: any, e: React.MouseEvent) => {
    showConfirmAlert("Bạn có chắc chắn muốn xóa phiếu sự cố này?").then(
      (res) => {
        if (res?.isConfirmed) {
          deleteIncidentMutation.mutate(incident);
        }
      },
    );
    e.stopPropagation();
  ...
  const handlePlanRowClick = (plan: MaintenancePlanData) => {
    ...
    setSelectedIncident(null);
    // Đồng bộ năm đang mở với plan được chọn để bảng compact hiển thị đúng dữ liệu
    if (plan.nam !== undefined && plan.nam !== expandedYear) {
      setYearPlansPage(0);
      setExpandedYear(plan.nam);
    }
  ...
  // ── Selection helpers ─────────────────────────────────────
  // Lưu ý: chỉ tính trên các plan hiện đang tải (trang/năm hiện tại)
  const selectedPlans = currentYearPlans.filter((plan) =>
  ...
  const serverCounts =
    (activeTab === 0
      ? (yearSummary?.data?.trangThaiCounts ?? yearSummary?.trangThaiCounts)
      : incidentPageData?.trangThaiCounts) || {};
  ...
  const isDetailOpen = !!selectedPlan || !!selectedIncident;
  ...
  const incidentColumnsFull = [
    { field: "soPhieu", headerName: "Số phiếu", width: 140, editable: false },
    {
      field: "tenHeThongThietBi",
      headerName: "Hệ thống/ thiết bị sự cố",
      flex: 1,
      minWidth: 200,
      editable: false,
    },
    {
      field: "mucDo",
      headerName: "Mức độ nghiêm trọng",
      flex: 1,
      minWidth: 200,
      editable: false,
      renderCell: (params: any) => showServerity(params.value),
    },
    {
    ...
    },
    {
      ...
      renderCell: (params: any) => {
        ...
        return (
          <>
            <IconButton
              disabled={incident.trangThai !== 0}
              onClick={(e) => handleEditIncidentClick(incident, e)}
            >
              <Edit
                color={incident.trangThai !== 0 ? "#afb6bdff" : "#1976d2"}
              />
            </IconButton>
            <IconButton
              disabled={incident.trangThai !== 0}
              onClick={(e) => handleDeleteIncidentClick(incident, e)}
            >
              <Trash2 color={incident.trangThai !== 0 ? "#afb6bdff" : "red"} />
            </IconButton>
          </>
        );
      },
    ...
    <>
      ...
      <Box sx={{ p: 2 }}>
        ...
        >
            ...
            >
              ...
              {activeTab === 0 && (
                <TableCustom
                  ...
                  rows={isDetailOpen ? currentYearPlans : []}
                  columns={columnsCollapsed}
                  total={isDetailOpen ? currentYearPlansTotal : yearTotal}
                  isCompact={isDetailOpen}
                  ...
                  customContent={
                    !isDetailOpen ? (
                      /* ── LIST MODE: accordion theo năm (API 1: chỉ danh sách năm + tổng số) ── */
                      <Stack
                        spacing={2}
                      ...
                      >
                        {isLoadingYears ? (
                          ...
                          yearItems.map(
                            ({ nam, soLuongKeHoach, soLuongThietBi }) => {
                              const isYearOpen = expandedYear === nam;
                              return (
                                <Accordion
                                  key={nam}
                                  expanded={isYearOpen}
                                  onChange={() => toggleYear(nam)}
                                  disableGutters
                                  sx={{
                                    mb: 1,
                                    border: "1px solid",
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <Typography
                                        ...
                                        sx={{ flexGrow: 1 }}
                                      >
                                        Tổng TB: {soLuongThietBi ?? 0}
                                      </Typography>
                                    </Box>
                                  </AccordionSummary>
                                  <AccordionDetails sx={{ p: 0 }}>
                                    {!isYearOpen ? null : isLoadingYearPlans ? (
                                      <Box sx={{ p: 2 }}>
                                        <Typography
                                          variant="body2"
                                          color="text.secondary"
                                      ...
                                      <>
                                                                    ...
                                                                    {incidentReports.map(
                                                                      (
                                                                        ...
                                                                        const isIncidentSelected =
                                                                          (
                                                                            selectedIncident as IncidenData | null
                                                                          )
                                                                            ?.id ===
                                                                          incident.id;
                                                                        return (
                                        ...
                                        {/* Phân trang kế hoạch trong năm (API 2) */}
                                        {currentYearPlansTotalPages > 1 && (
                                          <Box
                                            sx={{
                                              display: "flex",
                                          ...
                                          >
                                            <Pagination
                                              size="small"
                                              page={yearPlansPage + 1}
                                              count={currentYearPlansTotalPages}
                                              onChange={(_, p) =>
                                                setYearPlansPage(p - 1)
                                              }
                                            />
                                          </Box>
                                        )}
                        ...
                        {/* Phân trang danh sách năm (API 1) */}
                        {yearTotalPages > 1 && (
                          <Box
                            sx={{
                              display: "flex",
                          ...
                          >
                            <Pagination
                              size="small"
                              page={yearPage + 1}
                              count={yearTotalPages}
                              onChange={(_, p) => setYearPage(p - 1)}
                            />
                          </Box>
                        )}
                      </Stack>
              ...
              {activeTab === 1 && (
                <TableCustom
                  ...
                  rows={incidentPageData?.items.map(IncidentAdapter) || []}
                  columns={incidentColumnsFull}
                  total={incidentPageData?.totalItems}
                  onPaginationModelChange={setPaginationModel}
                  paginationModel={paginationModel}
                  isCompact={isDetailOpen}
                  highlightedId={selectedIncident?.id}
                  onRowClick={(params) => {
                    setSelectedIncident(params.row as IncidenData);
                    setSelectedPlan(null);
                  }}
                  statusOptions={statusOptions}
                  statusValue={statusFilter}
          ...
          {/* ══ RIGHT PANEL ═════════════════════════════════════ */}
          {isDetailOpen && (selectedPlan || selectedIncident) && (
            <Paper
              elevation={0}
              sx={{
                flex: 1,
            ...
            >
              {selectedIncident ? (
                ...
                <PlanDetailPanel plan={selectedPlan} onClose={handleCloseAll} />
              ) : null}
            </Paper>
          )}
        </Box>
