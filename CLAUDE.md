...
5. **Không có `.gitignore` nào trong repo** — cẩn thận khi tạo file mới (log, `.env` local, `target/`, `node_modules/`) để không vô tình đề xuất commit thêm rác hoặc secret mới. Việc thêm `.gitignore` là 1 thay đổi cấu trúc, cần hỏi người dùng trước khi làm (theo CLAUDE.md gốc mục 8 — không đổi chức năng/cấu trúc khi chưa được yêu cầu).
